...
class ErrorMixin(abc.ABC, BaseException):
    """Clase base de Errores y Excepciones propias.

    Example:

        >>> class MyError(ErrorMixin):
                msg_template = "Valor ``{value}`` no encontrado"
        >>> raise MyError(value="cencosud valorcito")
        (...)
        MyError: Valor cencosud valorcito no encontrado
...
class TableNotFound(ErrorMixin, NameError):
    msg_template = "Una de las tablas del schema no se encuentra"
